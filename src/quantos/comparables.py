...
class ComparableCompanyEngine:
            ...
            "distributions": [
                {
                    ...
                    "peers": [
                        {"snapshot_id": peer.peer_snapshot_id, "value": str(peer.value)}
                        for peer in item.peer_values
                    ],
                    "percentiles": [
                        {"percentile": str(point.percentile), "value": str(point.value)}
    ...
    @staticmethod
    def _multiple(peer: PeerSnapshot, kind: MultipleKind) -> Decimal | None:
    ...
    def _distribution(
        ...
        records = [
            PeerMultiple(peer.entity_id, peer.snapshot_id, kind, value)
            for peer in peers
            if (value := self._multiple(peer, kind)) is not None
        ]
        ...
        if len(records) < policy.minimum_peers_per_multiple:
            return MultipleDistribution(
                kind, DistributionStatus.INSUFFICIENT_PEERS, len(records),
                tuple(records), None, (), None,
            )
        ...
        return MultipleDistribution(
            ...
            minimum=values[0],
            percentiles=tuple(
                PercentileValue(p, _percentile(values, p))
    ...
    def _implied_range(
        ...
        if distribution.status is DistributionStatus.INSUFFICIENT_PEERS:
            return ImpliedValuationRange(
                ...
                (),
                "Not enough eligible peers for this metric.",
            )
        denominator = {
        ...
        if denominator <= 0:
            return ImpliedValuationRange(
                ...
                (),
                "Target denominator is non-positive for this valuation metric.",
            )
            ...
            if distribution.kind in {
                ...
                if item.value <= 0:
                    return ImpliedValuationRange(
                        ...
                        ImpliedValuationStatus.TARGET_NOT_APPLICABLE,
                        (),
                        "Non-positive peer FCF yield cannot support reciprocal valuation.",
                    )
                enterprise = None
                equity = denominator / item.value
