...
- **Now called QuantOS.** The project, the desktop app, the downloads (`QuantOS-Windows-x64.zip`, `QuantOS-macOS-arm64.dmg`, `QuantOS-Linux-x86_64.AppImage`, `quantos-live-usb-amd64.iso`) and the repository ([purysho/QuantOS](https://github.com/purysho/QuantOS)) all carry the new name. The data folder is now `QuantOS` (`QuantOS-data` in portable mode).
- **Execution replay fix.** Fills that happen at the same instant, such as a market order sweeping two price levels, are now always returned in execution order. Before, their order could depend on their content hashes.
- **Desktop app for Windows, macOS and Linux.** It opens a control center in your browser: first-run setup, one-button data updates with live progress, company analysis, a health check, and the terminal. Everything stays on your computer (loopback only, per-launch token). It is single-instance, and portable mode keeps data in a `QuantOS-data` folder next to the program.
